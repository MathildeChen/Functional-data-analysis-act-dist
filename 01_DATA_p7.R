# Script name: 01_DATA_p7.R
# 
# Author: M.Chen, Inserm, 2021
#
# Doing: Preparing data for cross-sectional models 
# -> for full population (N = 3782) (non-stratified models)
# -> for men (n = 2828) (stratified models)
# -> for women (n = 954) (stratified models)
# 
# Data include 
...
#--------------------------------------------------------------------
# Full population (N = 3782) (non-stratified models)

# > Individual activity distribution (for functional models)
tab_pa <- act %>%
  select(-f_i_2) %>%
  filter(stno %in% unique(tab_7_fin$stno)) %>% 
...
tab_cov <- tab_7_fin %>%
  ...
  mutate(
    ...
    A_age_conti_5 = (mage_s_i - mean(tab_7_fin$mage_s_i))/5,
  ...
  select(stno, 
         ...
         O_waking_time, 
         # Socio-demographics
         "A_sex" = "sex_2", A_age_conti_5, "A_ethnicity" = "ethnicity_i", "A_socio_eco_cont" = "mlgrlump_i_ordinal", "A_marital_status" = "mstatusx_i_2",
         # Behavioral covariates
...
YY_Activity <- as.matrix(data0[,c(2:151)],
                         nrow = length(unique(tab_7_fin$stno)), 
                         ncol = 150)
...
# > Mean age in men
mean_age_man   <- mean(tab_7_fin[which(tab_7_fin$sex_2 == 1),]$mage_s_i)

# > Covariates chez les hommes
tab_cov_man <- tab_7_fin %>%
  ...
  mutate(
    ...
    O_waking_time = dur_day_min_pla - 16*60,
    # Age (standardized, 5 years increment)
    A_age_conti_5 = (mage_s_i - mean_age_man)/5) %>%
  # Select and rename covariates
  select(stno, 
         # Confusion factor
         O_waking_time, 
...
# > Mean age in women
mean_age_woman   <- mean(tab_7_fin[which(tab_7_fin$sex_2 == 0),]$mage_s_i)

# > Covariates fro women
tab_cov_woman <- tab_7_fin %>%
  ...
  mutate(
    ...
    O_waking_time = dur_day_min_pla - 16*60,
    # Age (standardized, 5 years increment)
    A_age_conti_5 = (mage_s_i - mean_age_woman)/5) %>%
  # Select and rename covariates
  select(stno, 
         # Confusion factor
         O_waking_time, 
...
# > Group dataframes
full_data_7  <- list(lm = data0,       fm = data1)
data_man_7   <- list(lm = data0_man,   fm = data1_man)
data_woman_7 <- list(lm = data0_woman, fm = data1_woman)

# > Full
path <- "E:\\PC_FIXE\\Analysis\\01_ARTICLE_1\\05_FINAL_ANALYSES_FIGURES_2\\PHASE_7\\Data"
save(full_data_7,  file = paste0(path, "\\full_data_7.rda"))
save(data_man_7,   file = paste0(path, "\\data_man_7.rda"))
save(data_woman_7, file = paste0(path, "\\data_woman_7.rda"))
